...
export const typeOrmModuleOptions = {
  useFactory: async (
    ...
    synchronize: process.env.ENVIRONMENT === 'production' ? false : true, // true로 설정하면 TypeORM이 데이터베이스 스키마를 자동으로 생성합니다. 개발 환경에서만 사용하는 것이 좋습니다.
    autoLoadEntities: true, // (boolean): 애플리케이션에서 엔티티 모든 클래스를 자동으로 로드합니다.
    logging:
      process.env.ENVIRONMENT === 'production' ? false : ['query', 'error'], //  (boolean | string | string[]): 로깅을 활성화합니다. true를 지정하면 모든 쿼리가 로깅됩니다. 문자열을 전달하면 로깅 레벨을 지정할 수 있습니다.
    extra: {
      max: 100, // 최대 연결 개수
